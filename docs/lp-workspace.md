...
- Success is `{data: LpLeaderboard, error: null, meta: {mode, storage, collectionIntervalSeconds, maxSourceAgeSeconds, collector}}`. The API reads only the dedicated Neon row. It never starts an RPC collection or falls back to process memory, a Next cache, SQLite, demo data, or an expired observation. Missing/expired/unreadable storage returns 503 with null data and `Retry-After`.
- This is a **shared verified research snapshot, not a live feed**. The dedicated publisher runs about every two minutes. Both API and browser withhold an observation older than **5 minutes**. A stored source block and `observedAt` never advance merely because a page was refreshed or a collection failed.
- `public.lp_leaderboard_snapshots` holds one current observation, its source block/time, publish time, producer Git revision, and separate latest-attempt state. Publication uses parameterized JSON and an atomic increasing-block/non-regressing-time guard. Replaying the same block/hash does not renew its timestamps. Failures update only attempt state, never the accepted payload. A failed latest collection is disclosed alongside an observation only while that observation remains inside its hard TTL.
- Public refresh is storage-only (5-second database-request timeout, 15-second API budget, 20-second browser timeout). Refresh deduplicates in-flight clicks and honors retry cooldowns. It cannot consume public RPC quota.
- The independent LP collector retains the source validation, 650-ms general pacing, **one-at-a-time `eth_getLogs` with at least 1.5-second start spacing**, 10-second dispatched-network timeout, 90-second total scan deadline, and fail-closed rate-limit handling. Provider cooldowns persist across job processes in the LP-only local state directory; a cooldown skip is not a successful collection. Its shell wrapper holds `flock` and enforces a 115-second limit; the Windows wrapper has a 130-second limit. No provider switching or JSON-RPC batching is used.
- Windows task **RobinLPRefresh** invokes a dedicated Windows launcher, which runs Ubuntu user `mjeon` against `/home/mjeon/.local/share/robin-lp-worker`. Trigger interval: 2 minutes; overlap: `IgnoreNew`; task ceiling: 3 minutes. Runtime source is a clean pinned checkout and contains only the required database environment value. Existing RobinSync tasks remain unchanged. The PC and this user's WSL/session must be available for fresh collection; the page fails closed after the TTL if the collector is offline.
- The additive migration `0001_lp_snapshot.sql` creates only the LP-specific table. This rollout applies that exact SQL, not a wholesale `db:push` or replay of historical migrations. Check the production migration ledger before any later full migration run. To stop collection, disable only `RobinLPRefresh`; do not drop the table or alter unrelated tasks as an automatic rollback.
- Browser loads once, supports explicit Refresh, validates the payload, and removes expired rankings. No wallet signing, trading, Telegram notification, or write triggered by a public page request is performed.
