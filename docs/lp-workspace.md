...
- Success is `{data: LpLeaderboard, error: null}`; unavailable reads return 503 and `data: null`. Browser/edge response caching is disabled.
- One in-flight server read is shared per process; a successful snapshot is reused for at most 45 seconds and only within the 120-second source-block freshness limit. A failed refresh does not serve stale success.
