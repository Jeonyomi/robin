...
- The independent LP collector retains the source validation, 650-ms pacing, 10-second dispatched-network timeout, 90-second total scan deadline, and fail-closed rate-limit handling. Its shell wrapper holds `flock` and enforces a 115-second limit; the Windows wrapper has a 130-second limit. No provider switching or JSON-RPC batching is used.
