...
import { LpLeaderboardSchema, type LpLeaderboard, type LpLeader } from "../../src/lib/lp-leaders";
...
test("REAL public API auto-loads NFT leaders and opens actual details without financial writes", async ({ page }, testInfo) => {
  ...
  const responsePromise = page.waitForResponse((response) => new URL(response.url()).pathname === "/api/v1/lp-leaders", { timeout: 90_000 });
...
for (const kind of ["unavailable", "stale", "invalid"] as const) {
  test(`fixture: ${kind} response withholds the ranking`, async ({ page }) => {
    ...
    else await fixture(page, kind === "stale" ? testFixtureBoard({ observedAt: "2020-01-01T00:00:00.000Z" }) : { ...testFixtureBoard(), rows: [{ tokenId: "<img>" }] });
    await page.goto("/liquidity");
    await expect(page.getByRole("main").getByRole("alert")).toContainText(kind === "invalid" ? "Invalid" : kind === "stale" ? "stale" : "unavailable");
    await expect(rows(page)).toHaveCount(0);
...
test("fixture: observation expires with the clock without any background request", async ({ page }) => {
  ...
  await page.goto("/liquidity"); await expect(rows(page)).toHaveCount(2);
  await page.clock.fastForward(120_002);
  await expect(rows(page)).toHaveCount(0);
