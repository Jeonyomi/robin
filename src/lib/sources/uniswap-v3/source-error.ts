...
export function lpSourceDiagnostic(error: unknown) {
  ...
  for (let depth = 0; depth < 5 && current && typeof current === "object" && !seen.has(current); depth++) {
    ...
    const category = /too many requests|rate.?limit|429/i.test(text) ? "rate-limit" : /timeout|timed out|aborted/i.test(text) ? "timeout" : /metadata not found|historical|pruned/i.test(text) ? "historical-state" : /forbidden|403|access denied/i.test(text) ? "access-denied" : /invalid params|invalid argument/i.test(text) ? "invalid-request" : /fetch failed|connect|network|ENOTFOUND/i.test(text) ? "network" : item.name === "ZodError" ? "schema" : "other";
